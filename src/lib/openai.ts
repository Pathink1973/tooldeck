...
export async function generateCardMetadata(
  url: string,
  imageUrl?: string
): Promise<{ description: string; tags: string[]; icon_key: string }> {
  try {
    ...
    if (!response.ok) {
      ...
      toast.error(`Erro na analise IA: ${errMsg}`);
      return { description: 'Recurso web guardado', tags: ['sem-tag'], icon_key: 'web' };
    }

    return {
      description: data.description ?? 'Recurso web guardado',
      tags: Array.isArray(data.tags) && data.tags.length > 0 ? data.tags : ['sem-tag'],
      icon_key: data.icon_key ?? 'web',
    };
  } catch (error: any) {
    console.error('Error calling generate-card-meta:', error?.message || error);
    toast.error('Nao foi possivel contactar o servico de IA');
    return {
...
export const generateDescriptionAndTags = generateCardMetadata;
